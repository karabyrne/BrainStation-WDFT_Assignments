...
app.get('/pages/movie/:movieId', (req, res) => {
    /* EDIT CODE HERE ----------------------------------
    render pages/movie with data of specfic movie
    -------------------------------------------------- */
    // console.log("it's working");
    // console.log("movieId is = ", req.params.movieId );

    let movieId = req.params.movieId;

    for(var i = 0; i < movieData.length; i++){
  
      if(movieId === movieData[i].id){
  
          const movieIWant = movieData[i];

          // const movieKeys = Object.keys(movieData[i]);//["keysArray","movie"]
          ...
          return res.render("pages/movie", {movieIWant: movieIWant});
            
        }
    } 
    return res.send("That movie does not exist");
});


//  for...in loop
//   for(key in data){
//       return res.render(`Key is ${key}, value is ${data[key]}`)
//     }
